...
import { KEYWORDS } from "@/data/locations";
import ConsultationForm from "../../../[city]/[keyword]/ConsultationForm";

export const dynamicParams = false; // Only allow the pre-generated static params

export async function generateStaticParams() {
  return [
    { category: "furniture", keyword: "personalized-sofa-upholstery" },
    { category: "hospitality", keyword: "hospitality-hotel-fabric" },
    { category: "interior", keyword: "trade-only-interior-design-yardage" }
  ];
}
...
function getShortTitle(keyword: string, location: string): string {
  const full = `${keyword} in ${location} | Leslie's Weaving Studio`;
  if (full.length <= 60) return full;
  
  const shortBrand = `${keyword} in ${location} | Leslie's Weaving`;
  if (shortBrand.length <= 60) return shortBrand;
  
  let cleanKw = keyword.startsWith("Custom ") ? keyword.substring(7) : keyword;
  const noCustomBrand = `${cleanKw} in ${location} | Leslie's Weaving`;
  if (noCustomBrand.length <= 60) return noCustomBrand;
  
  const minimalBrand = `${cleanKw} in ${location} | Leslie's`;
...
export async function generateMetadata({
  ...
  const title = getShortTitle(kw.keyword, "the United States");
  const description = `Discover bespoke 100% cotton ${kw.keywordShort.toLowerCase()} available nationwide. Woven on our 72" Dobby loom with over 70 colors available for designers.`;
...
export default async function NationalFabricPage({
  params,
}: {
  params: Promise<{ category: string; keyword: string }>;
}) {
  ...
  const displayKeyword = kw.keyword;
  const displayLocation = "the United States";
  const character = "coastal luxury, modern spaces, and traditional design sensibility";
  const audience = "Interior Designers & Architects";
  const state = "US";

  const bodyParagraph1 = kw.p1Template(displayLocation, character, audience, state);
  const bodyParagraph2 = kw.p2Template(displayLocation, [], state);
  const closingSignal = kw.closingTemplate(displayLocation, [], "national", state);
        ...
        <div className="relative container max-w-[1600px] mx-auto px-6 pt-20 sm:pt-24">
          <div className="max-w-3xl p-8 md:p-10 rounded-2xl backdrop-blur-xl shadow-2xl" style={{ background: "oklch(0.12 0.02 55 / 0.35)", border: "1px solid oklch(0.95 0.01 75 / 0.15)" }}>
            <p className="text-xs font-semibold tracking-widest uppercase mb-6" style={{ color: "oklch(0.72 0.1 35)" }}>
              National &middot; Est. 2012 &middot; American Made &middot; 100% Cotton
            </p>
            <h1 className="font-display text-4xl sm:text-5xl lg:text-6xl font-bold italic leading-tight mb-6 drop-shadow-lg" style={{ color: "white", textShadow: "0 4px 32px rgba(0,0,0,0.5)" }}>
              Bespoke {displayKeyword.toLowerCase()} for projects nationwide.
            </h1>
            <p className="text-lg md:text-xl leading-relaxed drop-shadow-md" style={{ color: "oklch(0.95 0.01 75)" }}>
              Leslie's Weaving Studio delivers premium custom-woven 100% cotton fabrics to designers and showrooms across the country. Woven on our 72-inch computerized Dobby loom in Florida.
            </p>
          </div>
        </div>
          ...
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            <div>
              ...
              <h2 className="font-display text-4xl sm:text-5xl font-bold leading-tight mb-8" style={{ color: "oklch(0.22 0.025 55)" }}>
                Weaving custom yardage<br />for design projects.
              </h2>
              ...
              <p className="text-base leading-relaxed italic mb-8" style={{ color: "oklch(0.52 0.13 35)" }}>
                {closingSignal}
              </p>

              <div className="flex gap-4">
                <Link href="/locations" className="text-sm font-bold tracking-widest uppercase px-6 py-3 border transition-colors hover:bg-black hover:text-white" style={{ borderColor: "oklch(0.22 0.025 55)", color: "oklch(0.22 0.025 55)" }}>
                  Explore Locations
                </Link>
              </div>
            </div>
